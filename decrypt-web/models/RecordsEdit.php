 ...
 * Page class
 */
class RecordsEdit extends Records
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RecordsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "RecordsEdit";
    ...
    protected function validateForm()
    {
        ...
        if (in_array("images", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("DocumentsGrid");
        if (in_array("documents", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("AssociatedRecordsGrid");
        if (in_array("associated_records", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        // owner
        $this->owner->setDbValueDef($rsnew, $this->owner->CurrentValue, $this->owner->ReadOnly);

        // c_cates
        $this->c_cates->setDbValueDef($rsnew, $this->c_cates->CurrentValue, $this->c_cates->ReadOnly);

        // current_country
        $this->current_country->setDbValueDef($rsnew, $this->current_country->CurrentValue, $this->current_country->ReadOnly);

        // current_city
        $this->current_city->setDbValueDef($rsnew, $this->current_city->CurrentValue, $this->current_city->ReadOnly);

        // current_holder
        $this->current_holder->setDbValueDef($rsnew, $this->current_holder->CurrentValue, $this->current_holder->ReadOnly);
        ...
        // origin_region
        $this->origin_region->setDbValueDef($rsnew, $this->origin_region->CurrentValue, $this->origin_region->ReadOnly);

        // origin_city
        $this->origin_city->setDbValueDef($rsnew, $this->origin_city->CurrentValue, $this->origin_city->ReadOnly);

        // start_year
        $this->start_year->setDbValueDef($rsnew, $this->start_year->CurrentValue, $this->start_year->ReadOnly);

        // start_month
        $this->start_month->setDbValueDef($rsnew, $this->start_month->CurrentValue, $this->start_month->ReadOnly);

        // start_day
        $this->start_day->setDbValueDef($rsnew, $this->start_day->CurrentValue, $this->start_day->ReadOnly);

        // end_year
        $this->end_year->setDbValueDef($rsnew, $this->end_year->CurrentValue, $this->end_year->ReadOnly);

        // end_month
        $this->end_month->setDbValueDef($rsnew, $this->end_month->CurrentValue, $this->end_month->ReadOnly);

        // end_day
        $this->end_day->setDbValueDef($rsnew, $this->end_day->CurrentValue, $this->end_day->ReadOnly);

        // record_type
        $this->record_type->setDbValueDef($rsnew, $this->record_type->CurrentValue, $this->record_type->ReadOnly);

        // status
        $this->status->setDbValueDef($rsnew, $this->status->CurrentValue, $this->status->ReadOnly);

        // symbol_sets
        $this->symbol_sets->setDbValueDef($rsnew, $this->symbol_sets->CurrentValue, $this->symbol_sets->ReadOnly);

        // cipher_types
        $this->cipher_types->setDbValueDef($rsnew, $this->cipher_types->CurrentValue, $this->cipher_types->ReadOnly);

        // cipher_type_other
        $this->cipher_type_other->setDbValueDef($rsnew, $this->cipher_type_other->CurrentValue, $this->cipher_type_other->ReadOnly);

        // symbol_set_other
        $this->symbol_set_other->setDbValueDef($rsnew, $this->symbol_set_other->CurrentValue, $this->symbol_set_other->ReadOnly);

        // number_of_pages
        $this->number_of_pages->setDbValueDef($rsnew, $this->number_of_pages->CurrentValue, $this->number_of_pages->ReadOnly);

        // inline_cleartext
        $this->inline_cleartext->setDbValueDef($rsnew, $this->inline_cleartext->CurrentValue, $this->inline_cleartext->ReadOnly);

        // inline_plaintext
        $this->inline_plaintext->setDbValueDef($rsnew, $this->inline_plaintext->CurrentValue, $this->inline_plaintext->ReadOnly);

        // cleartext_lang
        $this->cleartext_lang->setDbValueDef($rsnew, $this->cleartext_lang->CurrentValue, $this->cleartext_lang->ReadOnly);

        // plaintext_lang
        $this->plaintext_lang->setDbValueDef($rsnew, $this->plaintext_lang->CurrentValue, $this->plaintext_lang->ReadOnly);

        // document_types
        $this->document_types->setDbValueDef($rsnew, $this->document_types->CurrentValue, $this->document_types->ReadOnly);

        // paper
        $this->paper->setDbValueDef($rsnew, $this->paper->CurrentValue, $this->paper->ReadOnly);

        // additional_information
        $this->additional_information->setDbValueDef($rsnew, $this->additional_information->CurrentValue, $this->additional_information->ReadOnly);

        // access_mode
        $this->access_mode->setDbValueDef($rsnew, $this->access_mode->CurrentValue, $this->access_mode->ReadOnly);

        // km_encoded_plaintext_type
        $this->km_encoded_plaintext_type->setDbValueDef($rsnew, $this->km_encoded_plaintext_type->CurrentValue, $this->km_encoded_plaintext_type->ReadOnly);

        // km_numbers
        $this->km_numbers->setDbValueDef($rsnew, $this->km_numbers->CurrentValue, $this->km_numbers->ReadOnly);

        // km_content_words
        $this->km_content_words->setDbValueDef($rsnew, $this->km_content_words->CurrentValue, $this->km_content_words->ReadOnly);

        // km_function_words
        $this->km_function_words->setDbValueDef($rsnew, $this->km_function_words->CurrentValue, $this->km_function_words->ReadOnly);

        // km_syllables
        $this->km_syllables->setDbValueDef($rsnew, $this->km_syllables->CurrentValue, $this->km_syllables->ReadOnly);

        // km_morphological_endings
        $this->km_morphological_endings->setDbValueDef($rsnew, $this->km_morphological_endings->CurrentValue, $this->km_morphological_endings->ReadOnly);

        // km_phrases
        $this->km_phrases->setDbValueDef($rsnew, $this->km_phrases->CurrentValue, $this->km_phrases->ReadOnly);

        // km_sentences
        $this->km_sentences->setDbValueDef($rsnew, $this->km_sentences->CurrentValue, $this->km_sentences->ReadOnly);

        // km_punctuation
        $this->km_punctuation->setDbValueDef($rsnew, $this->km_punctuation->CurrentValue, $this->km_punctuation->ReadOnly);

        // km_nomenclature_size
        $this->km_nomenclature_size->setDbValueDef($rsnew, $this->km_nomenclature_size->CurrentValue, $this->km_nomenclature_size->ReadOnly);

        // km_sections
        $this->km_sections->setDbValueDef($rsnew, $this->km_sections->CurrentValue, $this->km_sections->ReadOnly);

        // km_headings
        $this->km_headings->setDbValueDef($rsnew, $this->km_headings->CurrentValue, $this->km_headings->ReadOnly);

        // km_plaintext_arrangement
        $this->km_plaintext_arrangement->setDbValueDef($rsnew, $this->km_plaintext_arrangement->CurrentValue, $this->km_plaintext_arrangement->ReadOnly);

        // km_ciphertext_arrangement
        $this->km_ciphertext_arrangement->setDbValueDef($rsnew, $this->km_ciphertext_arrangement->CurrentValue, $this->km_ciphertext_arrangement->ReadOnly);

        // km_memorability
        $this->km_memorability->setDbValueDef($rsnew, $this->km_memorability->CurrentValue, $this->km_memorability->ReadOnly);

        // km_symbol_set
        $this->km_symbol_set->setDbValueDef($rsnew, $this->km_symbol_set->CurrentValue, $this->km_symbol_set->ReadOnly);

        // km_diacritics
        $this->km_diacritics->setDbValueDef($rsnew, $this->km_diacritics->CurrentValue, $this->km_diacritics->ReadOnly);

        // km_code_length
        $this->km_code_length->setDbValueDef($rsnew, $this->km_code_length->CurrentValue, $this->km_code_length->ReadOnly);

        // km_code_type
        $this->km_code_type->setDbValueDef($rsnew, $this->km_code_type->CurrentValue, $this->km_code_type->ReadOnly);

        // km_metaphors
        $this->km_metaphors->setDbValueDef($rsnew, $this->km_metaphors->CurrentValue, $this->km_metaphors->ReadOnly);

        // km_material_properties
        $this->km_material_properties->setDbValueDef($rsnew, $this->km_material_properties->CurrentValue, $this->km_material_properties->ReadOnly);

        // km_instructions
        $this->km_instructions->setDbValueDef($rsnew, $this->km_instructions->CurrentValue, $this->km_instructions->ReadOnly);
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("ImagesGrid");
            if (in_array("images", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "images"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("DocumentsGrid");
            if (in_array("documents", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "documents"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("AssociatedRecordsGrid");
            if (in_array("associated_records", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
