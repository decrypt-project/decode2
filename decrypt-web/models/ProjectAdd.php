...
class ProjectAdd extends Project
{
    ...
    // CSS class/style
    public $CurrentPageName = "ProjectAdd";
    ...
    public function setVisibility()
    {
        ...
        $this->locked_by->Visible = false;
        $this->lock_date->Visible = false;
    }
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("ProjectList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ProjectList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ProjectView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ProjectList") {
                            ...
                            $returnUrl = "ProjectList"; // Return list page content
                        }
                    }
    ...
    {
        $this->owner->DefaultValue = $this->owner->getDefault(); // PHP
        $this->owner->OldValue = $this->owner->DefaultValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'has_access' first before field var 'x_has_access'
        $val = $CurrentForm->hasValue("has_access") ? $CurrentForm->getValue("has_access") : $CurrentForm->getValue("x_has_access");
        if (!$this->has_access->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->has_access->Visible = false; // Disable update for API request
            } else {
                $this->has_access->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->has_access->CurrentValue = $this->has_access->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        // is_public
        $this->is_public->RowCssClass = "row";

        // owner
        $this->owner->RowCssClass = "row";

        // has_access
        $this->has_access->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $curVal = trim(strval($this->owner->CurrentValue));
            if ($curVal != "") {
                $this->owner->ViewValue = $this->owner->lookupCacheOption($curVal);
            } else {
                $this->owner->ViewValue = $this->owner->Lookup !== null && is_array($this->owner->lookupOptions()) && count($this->owner->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->owner->ViewValue !== null) { // Load from cache
                $this->owner->EditValue = array_values($this->owner->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter("`id`", "=", $this->owner->CurrentValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->owner->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCacheImpl($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
            ...
            // has_access
            $curVal = trim(strval($this->has_access->CurrentValue));
            if ($curVal != "") {
                $this->has_access->ViewValue = $this->has_access->lookupCacheOption($curVal);
            } else {
                $this->has_access->ViewValue = $this->has_access->Lookup !== null && is_array($this->has_access->lookupOptions()) && count($this->has_access->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->has_access->ViewValue !== null) { // Load from cache
                $this->has_access->EditValue = array_values($this->has_access->lookupOptions());
                if ($this->has_access->ViewValue == "") {
                    $this->has_access->ViewValue = $Language->phrase("PleaseSelect");
                }
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                ...
                $lookupFilter = $this->has_access->getSelectFilter($this); // PHP
                $sqlWrk = $this->has_access->Lookup->getSql(true, $filterWrk, $lookupFilter, $this, false, true);
                $conn = Conn();
                $config = $conn->getConfiguration();
                $config->setResultCacheImpl($this->Cache);
                $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                ...
                $arwrk = $rswrk;
                $this->has_access->EditValue = $arwrk;
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->has_access->Visible && $this->has_access->Required) {
            if ($this->has_access->FormValue == "") {
                $this->has_access->addErrorMessage(str_replace("%s", $this->has_access->caption(), $this->has_access->RequiredErrorMessage));
            }
        }
        ...
        if (in_array("record_in_project", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("FileInProjectGrid");
        if (in_array("file_in_project", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("DataInProjectGrid");
        if (in_array("data_in_project", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // has_access
        $this->has_access->setDbValueDef($rsnew, $this->has_access->CurrentValue, false);
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("RecordInProjectGrid");
            if (in_array("record_in_project", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->project_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "record_in_project"); // Load user level of detail table
            ...
            $detailPage = Container("FileInProjectGrid");
            if (in_array("file_in_project", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->project_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "file_in_project"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->project_id->setSessionValue(""); // Clear master key if insert failed
            ...
            $detailPage = Container("DataInProjectGrid");
            if (in_array("data_in_project", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->project_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "data_in_project"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->project_id->setSessionValue(""); // Clear master key if insert failed
