 ...
 * Page class
 */
class FileInProjectList extends FileInProject
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FileInProjectList";
    ...
    public $FormName = "ffile_in_projectlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "FileInProjectList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "FileInProjectAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "FileInProjectDelete";
        $this->MultiUpdateUrl = "FileInProjectUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "ffile_in_projectgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->filetype->AdvancedSearch->toJson(), ","); // Field filetype
        ...
        $filterList = Concat($filterList, $this->locked_by->AdvancedSearch->toJson(), ","); // Field locked_by
        $filterList = Concat($filterList, $this->lock_date->AdvancedSearch->toJson(), ","); // Field lock_date
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "ffile_in_projectsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field filetype
        $this->filetype->AdvancedSearch->SearchValue = @$filter["x_filetype"];
        $this->filetype->AdvancedSearch->SearchOperator = @$filter["z_filetype"];
        $this->filetype->AdvancedSearch->SearchCondition = @$filter["v_filetype"];
        $this->filetype->AdvancedSearch->SearchValue2 = @$filter["y_filetype"];
        $this->filetype->AdvancedSearch->SearchOperator2 = @$filter["w_filetype"];
        $this->filetype->AdvancedSearch->save();
        ...
        // Field locked_by
        $this->locked_by->AdvancedSearch->SearchValue = @$filter["x_locked_by"];
        $this->locked_by->AdvancedSearch->SearchOperator = @$filter["z_locked_by"];
        $this->locked_by->AdvancedSearch->SearchCondition = @$filter["v_locked_by"];
        $this->locked_by->AdvancedSearch->SearchValue2 = @$filter["y_locked_by"];
        $this->locked_by->AdvancedSearch->SearchOperator2 = @$filter["w_locked_by"];
        $this->locked_by->AdvancedSearch->save();

        // Field lock_date
        $this->lock_date->AdvancedSearch->SearchValue = @$filter["x_lock_date"];
        $this->lock_date->AdvancedSearch->SearchOperator = @$filter["z_lock_date"];
        $this->lock_date->AdvancedSearch->SearchCondition = @$filter["v_lock_date"];
        $this->lock_date->AdvancedSearch->SearchValue2 = @$filter["y_lock_date"];
        $this->lock_date->AdvancedSearch->SearchOperator2 = @$filter["w_lock_date"];
        $this->lock_date->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->project_id, $default, false); // project_id
        $this->buildSearchSql($where, $this->filename, $default, false); // filename
        $this->buildSearchSql($where, $this->path, $default, false); // path
        $this->buildSearchSql($where, $this->type, $default, false); // type
        $this->buildSearchSql($where, $this->filetype, $default, false); // filetype
        $this->buildSearchSql($where, $this->filesize, $default, false); // filesize
        ...
        $this->buildSearchSql($where, $this->locked_by, $default, false); // locked_by
        $this->buildSearchSql($where, $this->lock_date, $default, false); // lock_date
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->filename->AdvancedSearch->save(); // filename
            $this->path->AdvancedSearch->save(); // path
            $this->type->AdvancedSearch->save(); // type
            $this->filetype->AdvancedSearch->save(); // filetype
            $this->filesize->AdvancedSearch->save(); // filesize
            $this->creation_date->AdvancedSearch->save(); // creation_date
            $this->last_updated->AdvancedSearch->save(); // last_updated
            $this->locked_by->AdvancedSearch->save(); // locked_by
            $this->lock_date->AdvancedSearch->save(); // lock_date
    ...
    public function showFilterList()
    {
        ...
        // Field project_id
        $filter = $this->queryBuilderWhere("project_id");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->project_id, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->project_id->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field filename
        $filter = $this->queryBuilderWhere("filename");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->filename, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->filename->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field filesize
        $filter = $this->queryBuilderWhere("filesize");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->filesize, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->filesize->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field creation_date
        $filter = $this->queryBuilderWhere("creation_date");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->creation_date, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->creation_date->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field last_updated
        $filter = $this->queryBuilderWhere("last_updated");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->last_updated, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->last_updated->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field locked_by
        $filter = $this->queryBuilderWhere("locked_by");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->locked_by, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->locked_by->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field lock_date
        $filter = $this->queryBuilderWhere("lock_date");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->lock_date, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->lock_date->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        if ($this->BasicSearch->Keyword != "") {
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->path;
        $searchFlds[] = &$this->filetype;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->filename->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->path->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->type->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->filetype->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->filesize->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->locked_by->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->lock_date->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->filename->AdvancedSearch->unsetSession();
        $this->path->AdvancedSearch->unsetSession();
        $this->type->AdvancedSearch->unsetSession();
        $this->filetype->AdvancedSearch->unsetSession();
        $this->filesize->AdvancedSearch->unsetSession();
        $this->creation_date->AdvancedSearch->unsetSession();
        $this->last_updated->AdvancedSearch->unsetSession();
        $this->locked_by->AdvancedSearch->unsetSession();
        $this->lock_date->AdvancedSearch->unsetSession();
    }
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->filename->AdvancedSearch->load();
        $this->path->AdvancedSearch->load();
        $this->type->AdvancedSearch->load();
        $this->filetype->AdvancedSearch->load();
        $this->filesize->AdvancedSearch->load();
        $this->creation_date->AdvancedSearch->load();
        $this->last_updated->AdvancedSearch->load();
        $this->locked_by->AdvancedSearch->load();
        $this->lock_date->AdvancedSearch->load();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->last_updated); // last_updated
            $this->updateSort($this->locked_by); // locked_by
            $this->updateSort($this->lock_date); // lock_date
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"file_in_project\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"file_in_project\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffile_in_projectlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"ffile_in_projectlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"file_in_project\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("project_id", $this->createColumnOption("project_id"));
            $option->add("filename", $this->createColumnOption("filename"));
            $option->add("type", $this->createColumnOption("type"));
            $option->add("filesize", $this->createColumnOption("filesize"));
            $option->add("creation_date", $this->createColumnOption("creation_date"));
            $option->add("last_updated", $this->createColumnOption("last_updated"));
            $option->add("locked_by", $this->createColumnOption("locked_by"));
            $option->add("lock_date", $this->createColumnOption("lock_date"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffile_in_projectsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffile_in_projectsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffile_in_projectlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_file_in_project", "data-rowtype" => ROWTYPE_ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_file_in_project",
    ...
    protected function loadSearchValues()
    {
        ...
        // project_id
        if ($this->project_id->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->project_id->AdvancedSearch->SearchValue != "" || $this->project_id->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // filename
        if ($this->filename->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->filename->AdvancedSearch->SearchValue != "" || $this->filename->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // path
        if ($this->path->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->path->AdvancedSearch->SearchValue != "" || $this->path->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // filetype
        if ($this->filetype->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->filetype->AdvancedSearch->SearchValue != "" || $this->filetype->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // filesize
        if ($this->filesize->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->filesize->AdvancedSearch->SearchValue != "" || $this->filesize->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        // locked_by
        if ($this->locked_by->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->locked_by->AdvancedSearch->SearchValue != "" || $this->locked_by->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }

        // lock_date
        if ($this->lock_date->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->lock_date->AdvancedSearch->SearchValue != "" || $this->lock_date->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->project_id->EditValue = $this->project_id->AdvancedSearch->SearchValue;
            $this->project_id->PlaceHolder = RemoveHtml($this->project_id->caption());

            // filename
            $this->filename->setupEditAttributes();
            $this->filename->EditValue = HtmlEncode($this->filename->AdvancedSearch->SearchValue);
            ...
            $this->filesize->EditValue = $this->filesize->AdvancedSearch->SearchValue;
            ...
            $this->last_updated->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->last_updated->AdvancedSearch->SearchValue, $this->last_updated->formatPattern()), $this->last_updated->formatPattern()));
            $this->last_updated->PlaceHolder = RemoveHtml($this->last_updated->caption());

            // locked_by
            $this->locked_by->setupEditAttributes();
            $curVal = trim(strval($this->locked_by->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->locked_by->AdvancedSearch->ViewValue = $this->locked_by->lookupCacheOption($curVal);
            } else {
                $this->locked_by->AdvancedSearch->ViewValue = $this->locked_by->Lookup !== null && is_array($this->locked_by->lookupOptions()) && count($this->locked_by->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->locked_by->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->locked_by->EditValue = array_values($this->locked_by->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter("`id`", "=", $this->locked_by->AdvancedSearch->SearchValue, DATATYPE_NUMBER, "");
                }
            ...
            $this->lock_date->EditValue = HtmlEncode(FormatDateTime(UnFormatDateTime($this->lock_date->AdvancedSearch->SearchValue, $this->lock_date->formatPattern()), $this->lock_date->formatPattern()));
    ...
    protected function validateSearch()
    {
        ...
        if (!CheckInteger($this->project_id->AdvancedSearch->SearchValue)) {
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ffile_in_projectsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
