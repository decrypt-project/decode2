 ...
 * Page class
 */
class DocumentsGrid extends Documents
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocumentsGrid";
    ...
    public $FormName = "fdocumentsgrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "DocumentsGrid";
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_upload_date") && $CurrentForm->hasValue("o_upload_date") && $this->upload_date->CurrentValue != $this->upload_date->DefaultValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_category") && $CurrentForm->hasValue("o_category") && $this->category->CurrentValue != $this->category->DefaultValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_public_") && $CurrentForm->hasValue("o_public_") && $this->public_->CurrentValue != $this->public_->DefaultValue) {
            return false;
        }
        ...
        if ($CurrentForm->hasValue("x_uploader_id") && $CurrentForm->hasValue("o_uploader_id") && $this->uploader_id->CurrentValue != $this->uploader_id->DefaultValue) {
            return false;
        }
        return true;
    }
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_documents", "data-rowtype" => ROWTYPE_ADD]);
    ...
    {
        $this->upload_date->DefaultValue = $this->upload_date->getDefault(); // PHP
        $this->upload_date->OldValue = $this->upload_date->DefaultValue;
        $this->public_->DefaultValue = $this->public_->getDefault(); // PHP
        $this->public_->OldValue = $this->public_->DefaultValue;
        $this->uploader_id->DefaultValue = $this->uploader_id->getDefault(); // PHP
        $this->uploader_id->OldValue = $this->uploader_id->DefaultValue;
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->upload_date->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->upload_date->setFormValue($val);
            }
            $this->upload_date->CurrentValue = UnFormatDateTime($this->upload_date->CurrentValue, $this->upload_date->formatPattern());
        }
        if ($CurrentForm->hasValue("o_upload_date")) {
            $this->upload_date->setOldValue($CurrentForm->getValue("o_upload_date"));
        }
        ...
        if ($CurrentForm->hasValue("o_public_")) {
            $this->public_->setOldValue($CurrentForm->getValue("o_public_"));
        }
        ...
        if ($CurrentForm->hasValue("o_uploader_id")) {
            $this->uploader_id->setOldValue($CurrentForm->getValue("o_uploader_id"));
        }
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->upload_date->CurrentValue = FormatDateTime($this->upload_date->getDefault(), $this->upload_date->formatPattern());
            ...
            $this->uploader_id->CurrentValue = FormatNumber($this->uploader_id->getDefault(), $this->uploader_id->formatPattern());
    ...
    protected function addRow($rsold = null)
    {
        ...
        // upload_date
        $this->upload_date->setDbValueDef($rsnew, UnFormatDateTime($this->upload_date->CurrentValue, $this->upload_date->formatPattern()), strval($this->upload_date->CurrentValue) == "");

        // category
        $this->category->setDbValueDef($rsnew, $this->category->CurrentValue, false);

        // public_
        $this->public_->setDbValueDef($rsnew, $this->public_->CurrentValue, strval($this->public_->CurrentValue) == "");

        // record_id
        $this->record_id->setDbValueDef($rsnew, $this->record_id->CurrentValue, false);

        // uploader_id
        $this->uploader_id->setDbValueDef($rsnew, $this->uploader_id->CurrentValue, false);
