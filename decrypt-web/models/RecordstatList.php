 ...
 * Page class
 */
class RecordstatList extends Recordstat
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RecordstatList";
    ...
    public $FormName = "frecordstatlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "RecordstatList";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'recordstat';
        $this->TableName = 'recordstat';
        ...
        // Table object (recordstat)
        if (!isset($GLOBALS["recordstat"]) || get_class($GLOBALS["recordstat"]) == PROJECT_NAMESPACE . "recordstat") {
            $GLOBALS["recordstat"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "RecordstatAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "RecordstatDelete";
        $this->MultiUpdateUrl = "RecordstatUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'recordstat');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page => View page
                    ...
                    $result["view"] = $pageName == "RecordstatView"; // If View page, no primary button
                } else { // List page
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->century->AdvancedSearch->toJson(), ","); // Field century
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "frecordstatsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field century
        $this->century->AdvancedSearch->SearchValue = @$filter["x_century"];
        $this->century->AdvancedSearch->SearchOperator = @$filter["z_century"];
        $this->century->AdvancedSearch->SearchCondition = @$filter["v_century"];
        $this->century->AdvancedSearch->SearchValue2 = @$filter["y_century"];
        $this->century->AdvancedSearch->SearchOperator2 = @$filter["w_century"];
        $this->century->AdvancedSearch->save();
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->century); // century
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"frecordstatlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"frecordstatlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . " " . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("century", $this->createColumnOption("century"));
        }
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"frecordstatsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"frecordstatsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="frecordstatlist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_recordstat", "data-rowtype" => ROWTYPE_ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_recordstat",
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->century->setDbValue($row['century']);
    }
    ...
    protected function newRow()
    {
        ...
        $row['century'] = $this->century->DefaultValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // century
            $this->century->ViewValue = $this->century->CurrentValue;
            ...
            // century
            $this->century->HrefValue = "";
            $this->century->TooltipValue = "";
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"frecordstatsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
