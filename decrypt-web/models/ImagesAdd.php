 ...
 * Page class
 */
class ImagesAdd extends Images
{
    use MessagesTrait;
    ...
    // CSS class/style
    public $CurrentPageName = "ImagesAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("ImagesList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    $returnUrl = "ImagesList";
                    if (GetPageName($returnUrl) == "ImagesList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ImagesView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ImagesList") {
                            ...
                            $returnUrl = "ImagesList"; // Return list page content
                        }
                    }
    ...
    public function renderRow()
    {
        ...
        // processed
        $this->processed->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        if (in_array("transcriptions", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("TranscriptionsGrid");
            if (in_array("transcriptions", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->image_id->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "transcriptions"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->image_id->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
