 ...
 * Page class
 */
class ImagesGrid extends Images
{
    use MessagesTrait;
    ...
    public $FormName = "fimagesgrid";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "ImagesGrid";
    ...
    public function emptyRow()
    {
        ...
        if ($CurrentForm->hasValue("x_record_id") && $CurrentForm->hasValue("o_record_id") && $this->record_id->CurrentValue != $this->record_id->DefaultValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_last_modified") && $CurrentForm->hasValue("o_last_modified") && $this->last_modified->CurrentValue != $this->last_modified->DefaultValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_processed") && $CurrentForm->hasValue("o_processed") && $this->processed->CurrentValue != $this->processed->DefaultValue) {
            return false;
        }
        return true;
    }
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->processed->DefaultValue = $this->processed->getDefault(); // PHP
        $this->processed->OldValue = $this->processed->DefaultValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_record_id")) {
            $this->record_id->setOldValue($CurrentForm->getValue("o_record_id"));
        }
        ...
        if (!$this->last_modified->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->last_modified->setFormValue($val, true, $validate);
            }
            $this->last_modified->CurrentValue = UnFormatDateTime($this->last_modified->CurrentValue, $this->last_modified->formatPattern());
        }
        if ($CurrentForm->hasValue("o_last_modified")) {
            $this->last_modified->setOldValue($CurrentForm->getValue("o_last_modified"));
        }

        // Check field name 'processed' first before field var 'x_processed'
        $val = $CurrentForm->hasValue("processed") ? $CurrentForm->getValue("processed") : $CurrentForm->getValue("x_processed");
        if (!$this->processed->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->processed->Visible = false; // Disable update for API request
            } else {
                $this->processed->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_processed")) {
            $this->processed->setOldValue($CurrentForm->getValue("o_processed"));
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->last_modified->CurrentValue = UnFormatDateTime($this->last_modified->CurrentValue, $this->last_modified->formatPattern());
        $this->processed->CurrentValue = $this->processed->FormValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->record_id->getSessionValue() != "") {
                ...
                $this->record_id->OldValue = $this->record_id->CurrentValue;
                $this->record_id->ViewValue = $this->record_id->CurrentValue;
                $this->record_id->ViewValue = FormatNumber($this->record_id->ViewValue, $this->record_id->formatPattern());
            } else {
            ...
            // last_modified
            $this->last_modified->setupEditAttributes();
            $this->last_modified->EditValue = HtmlEncode(FormatDateTime($this->last_modified->CurrentValue, $this->last_modified->formatPattern()));
            $this->last_modified->PlaceHolder = RemoveHtml($this->last_modified->caption());

            // processed
            $this->processed->EditValue = $this->processed->options(false);
            $this->processed->PlaceHolder = RemoveHtml($this->processed->caption());
    ...
    protected function validateForm()
    {
        ...
        if (!CheckDate($this->last_modified->FormValue, $this->last_modified->formatPattern())) {
            $this->last_modified->addErrorMessage($this->last_modified->getErrorMessage(false));
        }
        if ($this->processed->Visible && $this->processed->Required) {
            if ($this->processed->FormValue == "") {
                $this->processed->addErrorMessage(str_replace("%s", $this->processed->caption(), $this->processed->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // last_modified
        $this->last_modified->setDbValueDef($rsnew, UnFormatDateTime($this->last_modified->CurrentValue, $this->last_modified->formatPattern()), $this->last_modified->ReadOnly);

        // processed
        $this->processed->setDbValueDef($rsnew, $this->processed->CurrentValue, $this->processed->ReadOnly);

        // Update current values
        $this->setCurrentValues($rsnew);
        if ($this->path->Visible && !$this->path->Upload->KeepFile) {
            $oldFiles = EmptyValue($this->path->Upload->DbValue) ? [] : explode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $this->path->htmlDecode(strval($this->path->Upload->DbValue)));
            if (!EmptyValue($this->path->Upload->FileName)) {
                $newFiles = explode(Config("MULTIPLE_UPLOAD_SEPARATOR"), strval($this->path->Upload->FileName));
        ...
        if ($updateRow) {
            ...
            if ($editRow) {
                if ($this->path->Visible && !$this->path->Upload->KeepFile) {
                    $oldFiles = EmptyValue($this->path->Upload->DbValue) ? [] : explode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $this->path->htmlDecode(strval($this->path->Upload->DbValue)));
                    if (!EmptyValue($this->path->Upload->FileName)) {
                        $newFiles = explode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $this->path->Upload->FileName);
                        $newFiles2 = explode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $this->path->htmlDecode($rsnew['path']));
                        $newFileCount = count($newFiles);
                        for ($i = 0; $i < $newFileCount; $i++) {
                            if ($newFiles[$i] != "") {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // last_modified
        $this->last_modified->setDbValueDef($rsnew, UnFormatDateTime($this->last_modified->CurrentValue, $this->last_modified->formatPattern()), strval($this->last_modified->CurrentValue) == "");

        // processed
        $this->processed->setDbValueDef($rsnew, $this->processed->CurrentValue, strval($this->processed->CurrentValue) == "");
        if ($this->path->Visible && !$this->path->Upload->KeepFile) {
            $oldFiles = EmptyValue($this->path->Upload->DbValue) ? [] : explode(Config("MULTIPLE_UPLOAD_SEPARATOR"), $this->path->htmlDecode(strval($this->path->Upload->DbValue)));
            if (!EmptyValue($this->path->Upload->FileName)) {
                $newFiles = explode(Config("MULTIPLE_UPLOAD_SEPARATOR"), strval($this->path->Upload->FileName));
