...
?>
<main class="edit">
<form name="ftoolconfigkeyedit" id="ftoolconfigkeyedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { toolconfigkey: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var ftoolconfigkeyedit;
...
<span id="el_toolconfigkey_tool_id">
    <select
        ...
        data-select2-id="ftoolconfigkeyedit_x_tool_id"
        <?php } ?>
...
<?php if (!$Page->tool_id->IsNativeSelect) { ?>
<script>
loadjs.ready("ftoolconfigkeyedit", function() {
    var options = { name: "x_tool_id", selectId: "ftoolconfigkeyedit_x_tool_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftoolconfigkeyedit.lists.tool_id?.lookupOptions.length) {
        options.data = { id: "x_tool_id", form: "ftoolconfigkeyedit" };
    } else {
        options.ajax = { id: "x_tool_id", form: "ftoolconfigkeyedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
    ...
    <input type="hidden" data-table="toolconfigkey" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ftoolconfigkeyedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ftoolconfigkeyedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
