...
?>
<main class="edit">
<form name="ffile_in_projectedit" id="ffile_in_projectedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { file_in_project: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var ffile_in_projectedit;
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_file_in_project_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_file_in_project_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="file_in_project" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["ffile_in_projectedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffile_in_projectedit", "x_creation_date", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["ffile_in_projectedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffile_in_projectedit", "x_last_updated", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_file_in_project_locked_by">
    <select
        ...
        data-select2-id="ffile_in_projectedit_x_locked_by"
        <?php } ?>
        data-table="file_in_project"
...
<?php if (!$Page->locked_by->IsNativeSelect) { ?>
<script>
loadjs.ready("ffile_in_projectedit", function() {
    var options = { name: "x_locked_by", selectId: "ffile_in_projectedit_x_locked_by" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ffile_in_projectedit.lists.locked_by?.lookupOptions.length) {
        options.data = { id: "x_locked_by", form: "ffile_in_projectedit" };
    } else {
        options.ajax = { id: "x_locked_by", form: "ffile_in_projectedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["ffile_in_projectedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffile_in_projectedit", "x_lock_date", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ffile_in_projectedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ffile_in_projectedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
