...
?>
<main class="edit">
<form name="ftoolconfigedit" id="ftoolconfigedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<span id="el_toolconfig_project_id">
    <select
        ...
        data-select2-id="ftoolconfigedit_x_project_id"
        <?php } ?>
...
<?php if (!$Page->project_id->IsNativeSelect) { ?>
<script>
loadjs.ready("ftoolconfigedit", function() {
    var options = { name: "x_project_id", selectId: "ftoolconfigedit_x_project_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftoolconfigedit.lists.project_id?.lookupOptions.length) {
        options.data = { id: "x_project_id", form: "ftoolconfigedit" };
    } else {
        options.ajax = { id: "x_project_id", form: "ftoolconfigedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_toolconfig_toolconfigkey_id">
    <select
        ...
        data-select2-id="ftoolconfigedit_x_toolconfigkey_id"
        <?php } ?>
        data-table="toolconfig"
...
<script>
loadjs.ready("ftoolconfigedit", function() {
    var options = { name: "x_toolconfigkey_id", selectId: "ftoolconfigedit_x_toolconfigkey_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftoolconfigedit.lists.toolconfigkey_id?.lookupOptions.length) {
        options.data = { id: "x_toolconfigkey_id", form: "ftoolconfigedit" };
    } else {
        options.ajax = { id: "x_toolconfigkey_id", form: "ftoolconfigedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php } ?>
    ...
    <input type="hidden" data-table="toolconfig" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ftoolconfigedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ftoolconfigedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
