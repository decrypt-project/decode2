...
?>
<main class="edit">
<form name="fcore_langedit" id="fcore_langedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<?php if ($Page->lang->Visible) { // lang ?>
    <div id="r_lang"<?= $Page->lang->rowAttributes() ?>>
        <label id="elh_core_lang_lang" class="<?= $Page->LeftColumnClass ?>"><?= $Page->lang->caption() ?><?= $Page->lang->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->lang->cellAttributes() ?>>
<span id="el_core_lang_lang">
<template id="tp_x_lang">
    <div class="form-check">
        <input type="radio" class="form-check-input" data-table="core_lang" data-field="x_lang" name="x_lang" id="x_lang"<?= $Page->lang->editAttributes() ?>>
...
<?php if ($Page->_content->Visible) { // content ?>
    <div id="r__content"<?= $Page->_content->rowAttributes() ?>>
        <label id="elh_core_lang__content" for="x__content" class="<?= $Page->LeftColumnClass ?>"><?= $Page->_content->caption() ?><?= $Page->_content->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->_content->cellAttributes() ?>>
<span id="el_core_lang__content">
<input type="<?= $Page->_content->getInputTextType() ?>" name="x__content" id="x__content" data-table="core_lang" data-field="x__content" value="<?= $Page->_content->EditValue ?>" maxlength="65535" placeholder="<?= HtmlEncode($Page->_content->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->_content->formatPattern()) ?>"<?= $Page->_content->editAttributes() ?> aria-describedby="x__content_help">
...
<?php } ?>
    ...
    <input type="hidden" data-table="core_lang" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?= $Page->IsModal ? '<template class="ew-modal-buttons">' : '<div class="row ew-buttons">' ?><!-- buttons .row -->
    <div class="<?= $Page->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fcore_langedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fcore_langedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
