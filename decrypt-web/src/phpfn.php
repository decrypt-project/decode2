...
function ConnectDb($info)
{
    ...
    if ($dbtype == "MYSQL") {
        ...
        if ($info["driver"] == "pdo_mysql") {
            ...
            if (defined("PDO::MYSQL_ATTR_SSL_VERIFY_SERVER_CERT") && ArrayKeyExists(fn($k) => in_array($k, $keys), $info["driverOptions"] ?? [])) { // SSL
                $info["driverOptions"][\PDO::MYSQL_ATTR_SSL_VERIFY_SERVER_CERT] ??= false;
            }
        } elseif ($info["driver"] == "mysqli") {
            if (ArrayKeyExists(fn($k) => StartsString("ssl_", $k), $info)) { // SSL
...
function GetMultiSearchSqlFilter($fldExpression, $fldOpr, $fldVal, $dbid, $sep)
{
    ...
    $sql .= $fldExpression . LikeOrNotLike($likeOpr, QuotedValue(Wildcard($fldVal . $sep, "STARTS WITH"), DATATYPE_STRING, $dbid), $dbid) . " " . $cond . " " .
        $fldExpression . LikeOrNotLike($likeOpr, QuotedValue(Wildcard($sep . $fldVal . $sep, "LIKE"), DATATYPE_STRING, $dbid), $dbid) . " " . $cond . " " .
        $fldExpression . LikeOrNotLike($likeOpr, QuotedValue(Wildcard($sep . $fldVal, "ENDS WITH"), DATATYPE_STRING, $dbid), $dbid);
    return $sql;
}
...
function SearchFilter($fldExpression, $fldOpr, $fldVal, $fldType, $dbid)
{
    ...
    if (in_array($fldOpr, ["=", "<>", "<", "<=", ">", ">="])) {
        ...
        $filter .= Like(QuotedValue(Wildcard($fldVal, $fldOpr), DATATYPE_STRING, $dbid), $dbid);
    } elseif (in_array($fldOpr, ["NOT STARTS WITH", "NOT LIKE", "NOT ENDS WITH"])) {
        $filter .= NotLike(QuotedValue(Wildcard($fldVal, $fldOpr), DATATYPE_STRING, $dbid), $dbid);
...
function QuotedValue($value, $fldType, $dbid = 0)
{
    ...
    switch ($dataType) {
        ...
        case DATATYPE_MEMO:
            $val = $removeXss ? RemoveXss($value) : $value;
        ...
        foreach ($ar as $keyword) {
            if ($keyword != "") {
                ...
                if ($keyword == "OR" && $searchType == "") {
                    ...
                    $wrk = $fld->VirtualExpression . Like(QuotedValue(Wildcard($keyword, "LIKE"), DATATYPE_STRING, $dbid), $dbid);
                } elseif ($fld->DataType != DATATYPE_NUMBER || is_numeric($keyword)) {
                    $wrk = $fld->BasicSearchExpression . Like(QuotedValue(Wildcard($keyword, "LIKE"), DATATYPE_STRING, $dbid), $dbid);
                }
                if ($wrk != "") {
                    $arSql[$j] = $wrk;
                    $arCond[$j] = $defCond;
