...
{
    $app->get('/listimages/{record}', function ($request, $response, $args) {
        $recordId = $args["record"];
        if (is_numeric($recordId)) {
            return $response->withJson(ExecuteJson("SELECT id FROM images WHERE record_id =" . $recordId)); 
        }
        else {
            return $response->withJson(["message" => "integer required"]);
        }
    });
    $app->get('/listdocuments/{record}', function ($request, $response, $args) {
        $recordId = $args["record"];
        if (is_numeric($recordId)) {
            return $response->withJson(ExecuteJson("SELECT id FROM documents WHERE record_id =" . $recordId)); 
        }
        else {
            return $response->withJson(["message" => "integer required"]);
        }
...
function myGetLang($txkey, $lang) {
	$sql = "SELECT content FROM core_lang WHERE txkey = '" . $txkey . "' AND lang = '" . $lang . "'";
	return ExecuteScalar($sql);
}

function myBox($text) {
	return "<div class='myTooltip'>" . $text . "</div>";
}

function myLargeBox($text) {
	return "<div class='callout callout-info'>" . $text . "</div>";
}

function myCurrentUserID() {
