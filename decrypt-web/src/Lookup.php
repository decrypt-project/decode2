...
class Lookup
{
    ...
    public function renderViewRow($row, $renderer = null)
    {
        ...
        foreach ($this->DisplayFields as $index => $name) {
            ...
            if ($displayField) {
                ...
                if (!EmptyString($viewValue) && !($sameTable && $name == $this->Field->Name && !in_array($displayField->DataType, [DATATYPE_DATE, DATATYPE_TIME]) && $displayField->OptionCount == 0)) { // Make sure that ViewValue is not empty and not self lookup field (except Date/Time) and not field with user values
    ...
    protected function getSqlPart($part = "", $isUser = true, $useParentFilter = true, $skipFilterFields = false)
    {
        ...
        if ($cnt > 0) {
            ...
            if ($val != "") {
                $val = strval($val);
                if ($linkField->DataType == DATATYPE_GUID && !CheckGuid($val)) {
    ...
    {
        return $this->getSearchExpression() . Like(QuotedValue(Wildcard($sv, "STARTS WITH"), DATATYPE_STRING, $dbid), $dbid);
    }
    ...
    protected function getModalSearchFilter($sv, $dbid)
    {
        ...
        foreach ($ar as $keyword) {
            if ($keyword != "") {
                $thisFilter = $this->getSearchExpression() . Like(QuotedValue(Wildcard($keyword, "LIKE"), DATATYPE_STRING, $dbid), $dbid);
